...
void thread_data_io(bool to_device, uint8_t* data, volatile uint32_t* len, volatile bool* finish) {
	static const char lut_s[2][5] = { "from", "to" };
	static const uint8_t lut_dir[2] = { XAXIDMA_DEVICE_TO_DMA, XAXIDMA_DMA_TO_DEVICE };
	static uint32_t *lut_max_len_ptr[2] = {
	...
	while (opt_len > 0) {
		...
		int Status = XAxiDma_SimpleTransfer(&AxiDma, (UINTPTR)ptr,
								this_len, dir);
		if (Status != XST_SUCCESS) {
			xil_printf("XAxiDma_SimpleTransfer %s device failed with %d, "
					   "%d bytes left\r\n", s, Status, opt_len);
			*finish = true;
			vTaskDelete(NULL);
			return;
		}
		while (XAxiDma_Busy(&AxiDma, dir)) taskYIELD();
		*len -= this_len;
		opt_len -= this_len;
		ptr += this_len;
	}
...
void IP_process(uint16_t width,
	...
	DataIoThreadParamPack in_param  = { .to_device = true,  .data = (uint8_t*)in, .len = total_len,  .finish = false };
	DataIoThreadParamPack out_param = { .to_device = false, .data = 		 out, .len = total_len, .finish = false };
	xTaskHandle in_thread, out_thread;

	BaseType_t status;
	status = xTaskCreate(TaskEntry, "IpDmaIn", THREAD_STACKSIZE, &in_param, DEFAULT_THREAD_PRIO, &in_thread);
	if (status != pdPASS) {
		xil_printf("xTaskCreate IpDmaIn failed with %d\r\n", status);
		return;
	}
	status = xTaskCreate(TaskEntry, "IpDmaOut", THREAD_STACKSIZE, &out_param, DEFAULT_THREAD_PRIO, &out_thread);
	if (status != pdPASS) {
		xil_printf("xTaskCreate IpDmaIn failed with %d\r\n", status);
		return;
	}
	while (!out_param.finish || !in_param.finish) taskYIELD();

	if (in_param.len != 0 || out_param.len != 0) {
		xil_printf("Transfer incomplete, want %u bytes, sent %u, received %u bytes\r\n",
