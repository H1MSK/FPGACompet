...
case class AxiStreamWidthAdaptorI8O32() extends Component {
  val io = new Bundle {
    val i = slave(Axi4Stream(Axi4StreamConfig(dataWidth = 8, useKeep = true, useLast = true)))
    val o = master(Axi4Stream(Axi4StreamConfig(dataWidth = 32, useKeep = true, useLast = true)))
  }
